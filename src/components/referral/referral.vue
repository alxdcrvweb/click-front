<template class="template">
    ...
    <div class="refContainerEmpty">
        ...
        <div class="squadd">
            ...
            <div>{{ profile?.referals }}</div>
        </div>
        <div class="update" @click="copyURL(getInviteLink(link[0]?.username), 'user')">
            <div>
                <div class="updateTitle">
                    👉 PERSONAL REF LINK</div>
                <div class="updateSubtitle">{{ getInviteLink(link[0]?.username), 'squad' }}</div>
            </div>
            <div>
        ...
        <div v-if="link[1]" class="update" @click="copyURL(getInviteLink(link[1]?.username), 'squad')">

            <div>
                <div class="updateTitle">
                    🤝 SQUAD REF LINK</div>
                <div class="updateSubtitle">{{ getInviteLink(link[1]?.username) }}</div>
            </div>
            <div>
                <img src="../../assets/plus.png" />
            </div>
...
export default {
    ...
    setup() {
        ...
        return { profile: getProfile, getLevel, getIsConnected, getCoins, jwt: getJwt, link, botname, copied, copied2 }
    },
    mounted() {
        this.getBot()
        this.getLinkRef()
    ...
    methods: {
        ...
        async getLinkRef() {
            ...
            link.value = res.link.links

        },

        async getBot() {
            const res = await getBotname(this.jwt)
            botname.value = res.botname.botname
        ...
        getInviteLink(username: string) {
            return `https://t.me/${botname.value}?start=${username}`
        },
