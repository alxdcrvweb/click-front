<template>
    ...
    <div v-if="profile == undefined" class="isLoading">
    ...
    <div v-else-if="stage == 4" class="frame__container" id="contain" :sftyle="{
        ...
        <div class="clicker">
            ...
            <div class="rowText" :style="{ 'opacity': currentBoostSize > 0 ? 1 : 0 }">⛓ BOOSTED ⛓</div>
            <!-- <div class="rocketContainer"
                ...
                <div @touchstart="boost" @touchend="handleTouchEnd" class="rocket"
                    :class="{ 'show': profile?.hasBoost && open && profile?.dailyClickBoosts !== 0 }">
...
export default {
    ...
    setup() {
        ...
        const { getLevel, getIsConnected, getCoins, getProfile, getStage, getUpdated } = storeToRefs(profileStore);
    ...
    methods: {
        boost() {
            open.value = false
            pos.value = "touched"
            if (this.animationFrameId !== null && currentBoostSize.value > 0) {
                cancelAnimationFrame(this.animationFrameId);
            }
            try {
                activateBoost(this.getJwt).then((res) => {
                    this.profile.dailyClickBoosts = this.profile.dailyClickBoosts - 1
                })
                currentBoostSize.value = this.getRandomBoostStrength()
                // this.profile.dailyClickBoosts = this.profile.dailyClickBoosts - 1
                // })
